...
import * as blazeface from '../../blazeface/index';
...
export interface MediaPipeFaceMesh {
  ...
  estimateFaces(config: EstimateFacesConfig): AnnotatedPrediction[];
}
...
class FaceMesh implements MediaPipeFaceMesh {
  ...
  estimateFaces(config: EstimateFacesConfig):
    ...
    if (predictions != null && predictions.length > 0) {
      return predictions.map((prediction: Prediction, i) => {
        ...
        const tensorValues = tensorsToRead.map((d: tf.Tensor) => d.arraySync());
        ...
        const [coordsArr, coordsArrScaled] = tensorValues.slice(1) as [Coords3D, Coords3D];
