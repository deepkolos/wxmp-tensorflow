...
export class BlazeFaceModel {
  ...
  getBoundingBoxes(
    ...
    if (!returnTensors) {
      boundingBoxes = boundingBoxes.map((boundingBox: tf.Tensor2D) => {
   ...
   */
   estimateFaces(
    ...
    annotateBoxes = true): NormalizedFace[] {
    ...
    const { boxes, scaleFactor } = this.getBoundingBoxes(
    ...
    return boxes.map((face: BlazeFacePrediction) => {
      const scaledBox =
      ...
      if (!annotateBoxes) {
        ...
        const [landmarkData, boxData, probabilityData] =
          [face.landmarks, scaledBox, face.probability].map(
            d => d.arraySync());
