...
export class HandPipeline {
  ...
  private getBoxForPalmLandmarks(
    ...
    const rotatedPalmLandmarks: Coords2D = palmLandmarks.map((coord: [number, number]): [
      number,
      number,
    ] => {
  ...
  private transformRawCoords(
    ...
    const coordsScaled: Array<[number, number, number]> = rawCoords.map(
    ...
    const coordsRotated: Array<[number, number, number]> = coordsScaled.map(
      (coord: [number, number, number]) => {
    ...
    return coordsRotated.map((coord: [number, number, number]): [number, number, number] => {
      return [coord[0] + originalBoxCenter[0], coord[1] + originalBoxCenter[1], coord[2]];
    });
  }

  estimateHand(image: tf.Tensor4D): Prediction | null {
  ...
  private shouldUpdateRegionsOfInterest(): boolean {
    ...
    return (
      roisCount !== this.maxHandsNumber || this.runsWithoutHandDetector >= this.maxContinuousChecks
    );
  }
}
