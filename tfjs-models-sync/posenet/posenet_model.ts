...
export class PoseNet {
  ...
  estimateMultiplePoses(
    ...
    const allTensorBuffers = toTensorBuffers3D(
  ...
  estimateSinglePose(
    ...
    const pose = decodeSinglePose(heatmapScores, offsets, outputStride);
    const poses = [pose];
  ...
  estimatePoses(
      ...
      LegacyMultiPersonInferenceConfig): Pose[] {
    if (config.decodingMethod === 'single-person') {
      const pose = this.estimateSinglePose(input, config);
      return [pose];
    } else {
