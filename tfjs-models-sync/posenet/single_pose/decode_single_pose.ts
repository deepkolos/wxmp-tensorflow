 ...
 * and position.
 */
export function decodeSinglePose(
    heatmapScores: tf.Tensor3D, offsets: tf.Tensor3D,
    outputStride: PoseNetOutputStride): Pose {
  let totalScore = 0.0;

  const heatmapValues = argmax2d(heatmapScores);

  const allTensorBuffers = [heatmapScores.bufferSync(), offsets.bufferSync(), heatmapValues.bufferSync()];
  ...
  const offsetPointsBuffer = offsetPoints.bufferSync();
